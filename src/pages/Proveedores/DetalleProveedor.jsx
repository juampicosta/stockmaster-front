...
import { MdEmail, MdPhone, MdAttachMoney, MdAccessTime, MdPerson, MdMonetizationOn, MdLocalShipping, MdInventory} from "react-icons/md";
    ...
    <div className="bg-gradient-to-br from-orange-50 to-white min-h-screen p-8">
      ...
      <div className="bg-white shadow-lg border border-yellow-200 rounded-xl p-6 mb-8">
        <div className="flex items-center mb-4">
          <MdPerson className="text-3xl text-orange-700 mr-3" />
          <span className="text-2xl font-semibold text-orange-800">{proveedor.razonSocial}</span>
        </div>
        <div className="flex items-center mb-2">
          <MdEmail className="text-xl text-orange-500 mr-2" />
          <span className="text-md text-gray-700">{proveedor.email}</span>
        </div>
        <div className="flex items-center">
          <MdPhone className="text-xl text-orange-500 mr-2" />
          <span className="text-md text-gray-700">{proveedor.telefono}</span>
        </div>
      </div>
          ...
          >
            ...
            <ul className="space-y-1 pl-7 text-md">
              <li className="flex items-center">
                ...
                <span><strong>Costo de Compra:</strong> {art.datosArticulo?.costoCompra}</span>
              </li>
              <li className="flex items-center">
                <MdLocalShipping className="text-lg text-blue-500 mr-2" />
                <span><strong>Costo de Pedido:</strong> {art.datosArticulo?.costoPedido}</span>
              </li>
              <li className="flex items-center">
                <MdAccessTime className="text-lg text-yellow-600 mr-2" />
                <span><strong>Demora de Entrega:</strong> {art.datosArticulo?.demoraEntrega}</span>
              </li>
              <li className="flex items-center">
                <MdAttachMoney className="text-lg text-green-700 mr-2" />
                <span><strong>Precio Unitario:</strong> {art.datosArticulo?.precioUnitario}</span>
              </li>
            </ul>
          </div>
        ))
