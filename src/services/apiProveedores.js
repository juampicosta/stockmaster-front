...
export const obtenerArticulosProveedor = async (idProveedor) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        data?.mensaje || 'Error al obtener los artículos del proveedor'
      )
    }
...
export const editArticuloProveedor = async (
  idProveedor,
  codigoArticulo,
  datos
) => {
  try {
    const response = await fetch(
      `${API_URL}/${idProveedor}/articulos/${codigoArticulo}`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      const data = await response.json()
      throw new Error(
        data?.mensaje || 'Error al editar el artículo del proveedor'
      )
    }
...
export const vincularArticuloProveedor = async (
  ...
  try {
    ...
    if (!response.ok) {
      const data = await response.json()
      throw new Error(
        data?.mensaje || 'Error al vincular el artículo al proveedor'
      )
    }
